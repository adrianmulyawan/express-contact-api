...
const updateContact = async (req, res) => {
  try {
    ...
    const updateContact = await contact.update({
      ...
      full_name: fullname || contact.full_name,
      email: email || contact.email,
      phone_number: phone_number || contact.phone_number
    });

    // > Bila terjadi kesalahan saat update data
    if (!updateContact) {
      return res.status(400).json({
        status: 'Failed',
        statusCode: 400,
        message: 'Something error where update process!',
      });
    }

    if (addresses && addresses.length > 0) {
      const contact_id = contact.addresses[0].contact_id;
      for (const address of addresses) {
    ...
    return res.status(400).json({
      status: 'Failed',
      statusCode: 400,
      message: 'Something Error in updateContact Controller!',
      error: error.message
    });
  }
};
